...
namespace Example._02910000000001_EvenI.Practice.E01.Example.Classes.Runtime.Example_02
{
	...
	class CE01List_DLinked_02_02<T> where T : IComparable
	{
		...
		public void InsertVal(int a_nIdx, T a_tVal)
		{
			...
			var oNode = this.CreateNode(a_tVal);
			var oNode_Prev = this.FindNode_At_Prev(a_nIdx);
		...
		private void RemoveNode(CNode a_oNode_Remove)
		{
			...
			var oNode_Prev = this.FindNode_Prev(a_oNode_Remove);
		...
		/** 직전 노드를 찾는다 */
		private CNode FindNode_Prev(CNode a_oNode)
		{
			var oNode = this.Node_Head;
			...
			while(oNode != null && oNode.Node_Next != a_oNode)
			{
				oNode = oNode.Node_Next;
			}
		...
		/** 직전 노드를 찾는다 */
		private CNode FindNode_At_Prev(int a_nIdx)
		{
			if(a_nIdx <= 0)
			{
				return null;
